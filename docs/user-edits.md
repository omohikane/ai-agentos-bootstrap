...
Every file links back to this page, and each row here links to its file.
...
| File | Purpose |
| --- | --- |
| [ansible/secrets.env.example](../ansible/secrets.env.example) | Template for `~/.env` (copied into user-data). Fill with **short-lived tokens only**. |
